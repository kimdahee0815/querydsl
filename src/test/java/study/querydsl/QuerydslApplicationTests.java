...
class QuerydslApplicationTests {
	...
	public void sort() throws Exception {
	    ...
	    //when
		List<Member> result = queryFactory.selectFrom(member).where(member.age.eq(100)).orderBy(member.age.desc(), member.username.asc().nullsLast())
				.fetch();
	...
	public void aggregation() throws Exception {
	    // given
		List<Tuple> result = queryFactory.select(member.count(), member.age.sum(), member.age.avg(), member.age.max(), member.age.min()).from(member)
				.fetch();
	...
	public void selectSubQuery() throws Exception {
		QMember memberSub = new QMember("memberSub");
	    // given
	    queryFactory.select(member.username, select(memberSub.age.avg()).from(memberSub))
				.from(member)
				.fetch();
	    //when
	    //then
	...
	@Test
	public void compileCase() throws Exception {
	    // given
		List<String> list = queryFactory.select(new CaseBuilder()
	...
	public void dynamicQuery_BooleanBuilder() throws Exception {
	    // given
		Member member= new Member("member1", 20, null);
	    em.persist(member);
